...
def cancel_stale_submissions(app_id):
    canceled = False
    for state in ["READY_FOR_REVIEW", "COMPLETING", "UNRESOLVED_ISSUES", "WAITING_FOR_REVIEW"]:
        ...
        for item in existing.get("data", []):
            try:
                api(
                    "PATCH",
                    f"/reviewSubmissions/{item['id']}",
                    json={"data": {"type": "reviewSubmissions", "id": item["id"], "attributes": {"canceled": True}}},
                )
                print(f"Canceled review submission {item['id']}")
                canceled = True
            except RuntimeError as exc:
                print(f"Could not cancel {item['id']}: {exc}")
    if canceled:
...
def submit_for_review(app_id, version_id):
    for attempt in range(6):
        try:
            ...
            api(
                "PATCH",
                f"/reviewSubmissions/{review_id}",
                json={"data": {"type": "reviewSubmissions", "id": review_id, "attributes": {"submitted": True}}},
            )
            print("Submitted for review")
            return
        except RuntimeError as exc:
            if "409" in str(exc) and attempt < 5:
...
def main():
    ...
    cancel_stale_submissions(app_id)
